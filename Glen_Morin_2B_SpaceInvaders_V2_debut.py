...
def affichagePlateau(lePlateau: Dict[str, int], leVaisseau: Dict[str, int], lesAliens: List[Dict[str, int]],aTire : bool,posy_aliensatteignable : int or None = None,tirS : int or None = None):
    
    """
    -----------
    Cette procedure va nous permettre tout au long du jeu d'afficher les differents elements de notre jeu comme les aliens, le vaiseau et les tires.
    -----------
    Parametre : On entre en Parametre le dictionnaire lePlateau, leVaisseau. On prend aussi la liste de dictionnaire lesAliens, aTire qui est un bouleen, la 
    posy_aliensatteignable qui est un entier et si aucune valeur n'est entrée alors on affectera None a cette variable et pour finir tirS qui definit le niveau du tir special libere qui est un entier 
    si aucun tir a ete libere alors elle sera initialisé a None
    -----------
...
def gestion_tir(leVaisseau : Dict,lesAliens : Dict,alienAtteignable : int)-> int:
    ...
    niv_tir_le_plus_eleve : int = 0
    
    #nombre d'aliens tues
    nbAliens_tues : int = 0
    for aliens in lesAliens:
        if aliens['posy']== alienAtteignable:
            if aliens['posx'] == leVaisseau['posx']:
                if aliens['tir'] != None:
                    if niv_tir_le_plus_eleve < aliens['tir']:
                        niv_tir_le_plus_eleve = aliens['tir']
                lesAliens.remove(aliens)
                nbAliens_tues +=1
     
    #niveau de tir maximal recuperer
    
    
    
    
    
    return nbAliens_tues, niv_tir_le_plus_eleve
...
if __name__ == "__main__":
    ...
    kb = SaisiCar()
    res : bool = False
    tir_special : int = None
    
    #Boucle principale du jeu
    while res != True:
        while partieFinie == False and action != "q":
            ...
            affichagePlateau(lePlateau, leVaisseau, lesAliens,aTire,alienAtteignable,tir_special)
            if aTire == True:
                nbAliens_tues,tir_special=gestion_tir(leVaisseau, lesAliens, alienAtteignable)
                score += nbAliens_tues
                lePlateau['score'] = score *5
            deplacerAliens(lesAliens, lePlateau)
            partieFinie = findelapartie(leVaisseau, lesAliens, lePlateau)
            time.sleep(0.005)           #Met en pause l'execution pour une duree de tp en seconde, prend en parametre un reel le temps en seconde.
            #os.system('clear')
            os.system('cls')
